;(function (scope) {
	...
	function findFactorions(endValue, number) {
		...
		var output = [],
			 ...
			 * Например 3! - 4! + (4 - 3) = -17, a нулевой элемент массива расчитывается исходя из 9! - 1! - 0! + (10 - 9)
			 * Таким образом при помощи этого массива можно найти разницу числа и суммы факториалов одного значения, зная эту разницу у предыдущего
			 * @constant {Number}[]
			 */
		...
		/** Проверка параметров */
		if ((endValue && typeof endValue !== 'number') || (number && typeof number !== 'number')) throw new TypeError('Wrong argument');
		...
		for (; number <= endValue; number = number + 1) {
			/**
			 * Запуск цикла анализа изменения цифр числа (по сравнению с предшествующим) и прибавления разницы, согласно массиву констант
